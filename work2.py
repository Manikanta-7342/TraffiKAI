...
from PyQt5.QtGui import QImage, QPalette, QBrush, QPixmap, QFont,QIcon
...
class TrafficUi(QMainWindow):    
    def __init__(self):
        ...
        self.east_label = QLabel("0", self)
        self.east_label.resize(60, 40)
        self.east_label.move(780, 357)
        self.east_label.setFont(newfont)
        self.east_label.setStyleSheet('border: 1.5px solid green;')

        self.south_label = QLabel("0", self)
        self.south_label.resize(50, 40)
        self.south_label.move(352, 825)
        self.south_label.setFont(newfont)
        self.south_label.setStyleSheet('border: 1.5px solid green;')

        self.west_label = QLabel("0", self)
        self.west_label.resize(60, 40)
        self.west_label.move(160, 358)
        self.west_label.setFont(newfont)
        self.west_label.setStyleSheet('border: 1.5px solid green;')

        self.north_label = QLabel("0", self)
        self.north_label.resize(50, 40)
        self.north_label.move(352, 175)
        self.north_label.setFont(newfont)
        self.north_label.setStyleSheet('border: 1.5px solid green;')

        self.east_density = QLabel("Low", self)  # east
        self.east_density.resize(100, 40)
        self.east_density.move(800, 600)
        self.east_density.setFont(newfont)
        self.east_density.setStyleSheet('color : green; border: 1.5px solid green;')

        self.south_density = QLabel("Low", self)
        self.south_density.resize(80, 40)
        self.south_density.move(605, 825)
        self.south_density.setFont(newfont)
        self.south_density.setStyleSheet('color : green;border: 1.5px solid green;')

        self.west_density = QLabel("Low", self)
        self.west_density.resize(100, 40)
        self.west_density.move(160, 600)
        self.west_density.setFont(newfont)
        self.west_density.setStyleSheet('color : green;border: 1.5px solid green;')

        self.north_density = QLabel("Low", self)
        self.north_density.resize(70, 40)
        self.north_density.move(605, 312)
        self.north_density.setFont(newfont)
        self.north_density.setStyleSheet('color : green;border: 1.5px solid green;')
    ...
    def case_if_low(self):

            self.east, self.south, self.west, self.north = 10, 20, 30, 40

            while self.east_status == "low" and self.west_status == "low" and self.north_status == "low" and self.south_status == "low":

                if self.cur == "east":
                    self.east_label.setText(str(self.east))
                    self.south_label.setText(str(self.south - 10))
                    self.west_label.setText(str(self.west - 10))
                    self.north_label.setText(str(self.north - 10))
                if self.cur == "south":
                    self.east_label.setText(str(self.east - 10))
    ...
    def runMethod_(self):
        ...
        self.file_emer = open('emer.txt', 'r+')
        self.file_read = self.file_emer.read()
        self.emer = self.file_read[1:-1]
        self.emer = [int(x) for x in self.emer.split(",")]
        self.cont_1 = self.emer.index(1) if 1 in self.emer else 'None'
        if self.cont_1==1:
            self.east,self.west,self.north,self.south = 120,60,90,30
            self.cur = "south"
        elif self.cont_1==2:
            self.east, self.west, self.north, self.south = 90,30,60,120
            self.cur = "west"
        elif self.cont_1==3:
            self.east, self.west, self.north, self.south = 60, 120, 30, 90
            self.cur = "north"
        else:
            self.cur = "east"
        ...
        while(True):
                ...
                if self.east_status == "low" and self.west_status == "low" and self.north_status == "low" and self.south_status == "low":
                    self.case_if_low()
                    self.east = 30
                    self.south = 60
                    self.west = 90
